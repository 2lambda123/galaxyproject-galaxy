...
export function useInstanceTesting<T extends TemplateSummary>(
    ...
    async function test(templateSummary: T) {
        testResults.value = undefined;
        testingError.value = undefined;
        showTestResults.value = true;
        try {
            const { data } = await testInstance(templateSummary.id.toString());
