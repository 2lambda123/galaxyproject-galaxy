...
const {
    ...
    (payload: TestUpdateInstancePayload) =>
        testUpdate({ user_file_source_id: props.instanceId.toString(), ...payload }),
    (payload: UpdateInstancePayload) => update({ user_file_source_id: props.instanceId.toString(), ...payload }),
    useInstanceRouting
);
</script>
<template>
