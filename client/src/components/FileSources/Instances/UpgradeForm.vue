...
const {
    ...
    (payload: TestUpgradeInstancePayload) =>
        testUpdate({ user_file_source_id: props.instance.id.toString(), ...payload }),
    (payload: UpgradeInstancePayload) => update({ user_file_source_id: props.instance.id.toString(), ...payload }),
    useInstanceRouting
);
</script>
<template>
