...
const {
    ...
    (payload: TestUpdateInstancePayload) =>
        testUpdate({ user_object_store_id: props.instanceId.toString(), ...payload }),
    (payload: UpdateInstancePayload) => update({ user_object_store_id: props.instanceId.toString(), ...payload }),
    useInstanceRouting
);
</script>
<template>
