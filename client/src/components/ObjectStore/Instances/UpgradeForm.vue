...
const {
    ...
    (payload: TestUpgradeInstancePayload) =>
        testUpdate({ user_object_store_id: props.instance.id.toString(), ...payload }),
    (payload: UpgradeInstancePayload) => update({ user_object_store_id: props.instance.id.toString(), ...payload }),
    useInstanceRouting
);
</script>
<template>
